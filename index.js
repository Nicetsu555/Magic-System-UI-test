...
function createDashboard(optionsMenu) {
    ...
    const DASHBOARD_CALL_AMBIENT_TAG = /\[\s*sfx\s*:\s*([^\]]{1,120})\]/i;
    ...
    const CALL_AMBIENT_COOLDOWN_TURNS = 2;
    ...
    function extractCallAmbientCue(text) {
        ...
        const match = String(text).match(DASHBOARD_CALL_AMBIENT_TAG);
        ...
        return {
            cue,
            rest: text.replace(DASHBOARD_CALL_AMBIENT_TAG, ' ')
        };
    ...
    function logCallAmbientCue(characterName, cue) {
        ...
        if ((callState.ambientCooldown || 0) > 0) {
    ...
    function buildPhoneReplyPrompt(characterName, mode, strict) {
        ...
        const lines = [
            ...
            '- ผลลัพธ์ทั้งหมดคือสิ่งที่พิมพ์ลงแอปแชทหรือพูดในสายเท่านั้น ห้ามมีคำบรรยายเจือปนแม้แต่ประโยคเดียว' +
                (isCall ? ' (ยกเว้นแท็ก [SFX: ...] ตามกติกาพิเศษด้านล่างเท่านั้น)' : ''),
            '- ห้ามบรรยายฉาก ท่าทาง สีหน้า ความรู้สึก อากาศ เวลา หรือสิ่งแวดล้อมใด ๆ ทั้งสิ้น ไม่ว่าจะมีเครื่องหมาย *ดอกจัน* หรือไม่ก็ตาม' +
                (isCall
                    ? ' ข้อยกเว้นเดียวคือเสียงพื้นหลังผ่านแท็ก [SFX: ...] เท่านั้น ห้ามบรรยายด้วยวิธีอื่นเด็ดขาด'
            ...
            '- ห้ามใส่ชื่อผู้พูดนำหน้า เครื่องหมายคำพูดครอบทั้งข้อความ แท็ก หรือคำอธิบายใด ๆ' +
                (isCall ? ' (นอกจาก [SFX: ...] ที่ขึ้นต้นได้ตามกติกาด้านล่าง)' : ''),
        ...
        if (isCall) {
            ...
            lines.push(
                ...
                'กติกาพิเศษเฉพาะตอนโทร (ห้ามใช้ตอนพิมพ์แชท): ถ้าฉากรอบตัว ' +
                    ...
                    characterName +
                    ' แท็กนี้ต้องอธิบายเสียงแวดล้อมล้วน ๆ เท่านั้น ห้ามใส่ท่าทาง สีหน้า หรือการกระทำของ ' +
                    characterName +
    ...
    // Whether {{user}}'s own typed line, inside the texting thread
    // itself, is asking outright to be called. This thread's reply
    // generation (requestPhoneGeneration/buildPhoneReplyPrompt) is a
    // fully separate pipeline from the main story's [CALL] tag
    // system — it never sees that instruction and is explicitly
    // forbidden from emitting any tag at all — so without a check
    // like this, a request typed here only ever gets a text back
    // promising a call that nothing goes on to actually place. Kept
    // deliberately broad (root "โทร" alone catches "โทรหา", "โทรมา",
    // "โทรกลับ", "อยากคุยโทรศัพท์", etc., however it's phrased) since
    // catching the many natural phrasings matters more than
    // precision here.
    const PHONE_CALL_REQUEST_PATTERN =
        /โทร|call\s*me|give.*\bcall\b|ring\s*me/i;

    function looksLikeCallRequest(text) {
        return PHONE_CALL_REQUEST_PATTERN.test(text);
    }

    // How long after the "I'll call you" text lands before the phone
    // actually starts ringing. Long enough that it doesn't read as
    // the text and the ring happening in the same instant, nowhere
    // near the multi-minute wait a request used to get by accident
    // when nothing but the spontaneous-call watcher's own poll
    // interval (SPONTANEOUS_CALL_CHECK_INTERVAL_MS, checked only
    // sometimes) was ever going to answer it.
    ...
    async function sendChatMessage() {
        ...
        // A direct request to be called still gets a normal text
        // reply first — {{char}} actually saying "okay, calling now"
        // matters, it's the whole thing that was missing before — but
        // then, rather than leaving it at a promise nothing acts on,
        // the phone genuinely rings a few seconds later on its own.
        const isCallRequest = looksLikeCallRequest(text) && !isCallOpen();
