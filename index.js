...
function createDashboard(optionsMenu) {
    ...
    // Pulls the spoken part out of a narrated story line: whatever
    // sits inside quote marks, or — if there are none — the line
    // with its asterisked stage directions removed.
    function extractSpokenLines(text) {
        ...
        return cleanPhoneReply(text);

    }

    // Scrubs a generated reply down to plain spoken text: no
    // asterisked narration, no stray markup or tags, no "Name:"
    // prefix, no wrapping quotes, and never more than a few lines.
    function cleanPhoneReply(text, characterName) {
    ...
    function buildPhoneReplyPrompt(characterName, mode) {
        ...
        return [
            '[ระบบโทรศัพท์ — อย่าเขียนต่อเนื้อเรื่องหลัก]',
            situation,
            '',
            'กติกาที่ต้องทำตามอย่างเคร่งครัด:',
            '- ตอบเป็นคำพูดล้วน ๆ เหมือนแชทมือถือจริง',
            '- ห้ามบรรยายท่าทาง ฉาก ความรู้สึก หรือใส่ข้อความในเครื่องหมาย *ดอกจัน* เด็ดขาด',
            '- ห้ามใส่ชื่อผู้พูด เครื่องหมายคำพูด แท็ก หรือคำอธิบายใด ๆ',
            `- สั้น กระชับ 1-3 บรรทัด ในน้ำเสียงและนิสัยของ ${characterName}`,
    ...
    // Runs the generation itself. SillyTavern changed
    // generateQuietPrompt's signature between versions (positional
    // arguments in older builds, one options object in newer ones),
    // so the shape is picked from the function's own arity instead
    // of guessing.
    async function requestPhoneGeneration(characterName, mode) {
        ...
        const prompt = buildPhoneReplyPrompt(characterName, mode);
        ...
        return cleanPhoneReply(raw, characterName);
