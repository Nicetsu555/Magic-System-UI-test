...
function createDashboard(optionsMenu) {
    ...
    function openChatThread(item) {
        ...
        if (chatThreadStatusDot) {
            ...
            chatThreadStatusDot.classList.toggle(
                'is-system',
                isSystem
            );
        ...
        chatThreadAvatar.classList.toggle(
            'system-avatar',
            isSystem
        );
        ...
        return `
            <div class="chat-row chat-row-in">
                ...
                <div
                    class="
                        chat-row-avatar
                        ${activeChatIsSystem ? 'system-avatar' : ''}
                    "
                >
    ...
    const DASHBOARD_CALL_START_TAG =
        /\[\s*(?:call|โทร)\s*\]/i;

    const DASHBOARD_CALL_END_TAG =
        /\[\s*(?:hangup|end ?call|วางสาย)\s*\]/i;
    ...
    function stripPhoneEventTags(text) {
        ...
        return text
            .replace(
                DASHBOARD_CALL_START_TAG,
                ''
            )
            .replace(
                DASHBOARD_CALL_END_TAG,
                ''
            );
    ...
    async function sendToRealChat(text) {
        ...
        if (
            ...
            throw new Error(
                'ไม่พบช่องพิมพ์ข้อความหลักของ SillyTavern'
            );
    ...
    async function requestGeneration() {
        ...
        if (
            context &&
            typeof context.executeSlashCommandsWithOptions ===
                'function'
        ) {
    ...
    const DASHBOARD_PHONE_TAG_PROMPT_KEY =
        'dashboardPhoneTags';
    ...
    function installPhoneTagInstruction() {
        ...
        try {
            ...
            const positions =
                context.extension_prompt_types || {};

            const roles =
                context.extension_prompt_roles || {};
    ...
    // =====================================================
    // CALL CAPTION NAVIGATION
    // =====================================================

    if (callScreenCaptions) {

        callScreenCaptions.addEventListener(
            'click',
            (event) => {
                ...
                const pages =
                    callState.captionPages || [];

                const direction =
                    button.dataset.callCaption;

                if (
                    direction === 'prev'
                ) {

                    callState.captionPage =
                        Math.max(
                            0,
                            (callState.captionPage || 0) - 1
                        );
                ...
                // เปลี่ยนหน้าทันที
                // ไม่เล่น typewriter ซ้ำ
                paintCallCaptionPage(false);
    ...
    // =====================================================
    // SPLIT LONG CALL TEXT INTO PAGES
    // =====================================================

    function splitCallCaptionPages(
        text,
        maxChars = 260
    ) {
    ...
    // =====================================================
    // PAINT ONE CALL CAPTION PAGE
    // =====================================================

    function paintCallCaptionPage(
        animate = false
    ) {
        ...
        const from =
            callState.captionFrom ||
            'in';


        // หยุด animation เก่าก่อน
        if (
            callState.captionTimer
        ) {
        ...
        callScreenCaptions.innerHTML = `
            ...
            >
                ...
                <div
                    class="call-screen-caption-nav"
                >

                    <button
                        type="button"
                        class="call-caption-nav-button"
                        data-call-caption="prev"
                        ${
                            pageIndex === 0
                                ? 'disabled'
                                : ''
        ...
        const subtitleText =
            callScreenCaptions.querySelector(
                '.subtitle-text'
            );
        ...
        // เวลากดย้อนกลับ/ต่อไป
        // หรือเป็นข้อความของผู้ใช้
        // ให้แสดงข้อความทันที
        if (
            !animate ||
            from !== 'in'
    ...
    function renderCallCaptions() {
        ...
        const captionKey =
            `${duringCall.length}:${latest.from}:${text}`;


        // ไม่ render ข้อความเดิมซ้ำ
        if (
            captionKey ===
            callState.lastCaptionKey
        ) {
        ...
        callState.captionPages =
            splitCallCaptionPages(
                text
            );


        // ข้อความใหม่เริ่มหน้าแรกเสมอ
        callState.captionPage =
            0;
    ...
    // ระหว่าง AI กำลังสร้างข้อความ
    // ไม่สร้าง Bubble ปลอม
    ...
    // Keeps long call replies inside a fixed bubble. A reply is split
    // into readable pages, and the reader moves through it manually.
    function renderCallCaptions() {
        ...
        callState.captionPages = splitCallCaptionPages(text);
        callState.captionPage = 0;
        callState.captionFrom = latest.from;

        paintCallCaptionPage(true);
    ...
    // After the call connects we show a stable status instead of a timer.
    function updateCallTimer() {

        if (callScreenStatus) {
            callScreenStatus.textContent = 'เชื่อมต่อแล้ว';
        }
