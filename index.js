...
function startGeneratedDialTone() {
    ...
    const ringOnce = () => {
        ...
        // Two overlapping tones give the classic warble instead of
...
function createDashboard(optionsMenu) {
    ...
    function buildCallConnectPrompt(characterName) {
    ...
    async function requestPhoneGeneration(characterName, mode) {
        ...
        const isCall = mode === 'call' || mode === 'call-connect';
        let ambientCue = '';
        ...
        try {
            ...
            firstRaw = await runQuietPhonePrompt(
                context,
                mode === 'call-connect'
                    ? buildCallConnectPrompt(characterName)
