...
function createDashboard(optionsMenu) {
    ...
    function recordCallLogEntry(characterName, connectedAt, wasIncoming, startIndex) {
    ...
    function buildCallConnectPrompt(characterName, userDialed) {

        const lines = [
            '[OOC — ระบบโทรศัพท์: นี่คือแอปแชท/โทรศัพท์ ไม่ใช่เนื้อเรื่องหลัก อ่านเพื่อทราบบริบทเท่านั้น ห้ามเขียนต่อเนื้อเรื่องหลักในคำตอบนี้]',
            userDialed
                ? `ตอนนี้ {{user}} กดโทรหา ${characterName} ในแอปโทรศัพท์ และ ${characterName} เพิ่งกดรับสายเมื่อกี้วินาทีนี้เอง คนที่ต้องพูดคนแรกคือคนที่รับสาย คือ ${characterName}`
            ...
            '- ผลลัพธ์คือสิ่งที่พูดในสายเท่านั้น ห้ามมีคำบรรยายเจือปนแม้แต่ประโยคเดียว (ยกเว้นแท็ก [SFX: ...] ถ้ามีเสียงพื้นหลังที่สมจริงจริง ๆ เท่านั้น)',
    ...
    async function requestPhoneGeneration(characterName, mode) {
        ...
        const isConnectLine =
            mode === 'call-connect' || mode === 'call-answer';
        let ambientCue = '';
    ...
    async function connectCall() {
        ...
        try {
            ...
            const line = await requestPhoneGeneration(name, 'call-answer');
    ...
    function describeCallLog(log) {
        ...
        if (log.status === 'answered') {
            ...
            return `${log.direction === 'in' ? 'สายเข้า' : 'สายออก'} • ${formatCallDuration(log.seconds || 1)}`;
