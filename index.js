...
function createDashboard(optionsMenu) {
    ...
    // inside quote marks (straight or curly). Returns '' if there's
    // no quoted speech to find.
    ...
    function extractSpokenLines(text) {
        ...
        const quoted = extractQuotedSpeech(text);
    ...
    // noise, prefers actual quoted speech if the model wrote any,
    // otherwise falls back to the plain leftover text — always
    // capped to a handful of short lines, never a narrated block.
    function sanitizePhoneText(text, characterName) {
        ...
        const quoted = extractQuotedSpeech(stripped);
    ...
    // True when a sanitized line still reads like narration rather
    // than something typed into a phone: too long, still carrying a
    // blank-line paragraph break, or referring to the character by
    // name (a person doesn't narrate themselves in third person in
    // their own text message).
    function looksLikeNarration(text, characterName) {

        if (!text) {
            return false;
        }

        if (text.length > PHONE_NARRATION_LENGTH_LIMIT) {
            return true;
        }
    ...
    // only, and both are told to stay consistent with the main
    // story happening right now. `strict` tightens the wording for
    // a retry, after a first attempt came back narrated anyway.
    function buildPhoneReplyPrompt(characterName, mode, strict) {
        ...
        const lines = [
            '[ระบบโทรศัพท์ — นี่คือแอปแชท/โทรศัพท์ ไม่ใช่เนื้อเรื่องหลัก]',
            situation,
            '',
            ...
            '- ห้ามเปลี่ยนไปคุยเรื่องอื่นที่ไม่เกี่ยวกับเรื่องหลัก',
            '',
            'ตัวอย่างคำตอบที่ถูกต้อง: "อยู่หน้าลานจอดรถแล้วนะ เดี๋ยวรอตรงนี้เลย"',
        ...
        if (strict) {
            ...
            lines.push(
                '',
                'คำตอบก่อนหน้านี้ยังมีคำบรรยายปนอยู่ ซึ่งผิดกติกา เขียนใหม่อีกครั้งเป็นคำพูดล้วน ๆ สั้น ๆ เท่านั้น ห้ามมีคำบรรยายใด ๆ แม้แต่คำเดียว'
            );
    ...
    // only if the result still looks narrated — asks exactly once
    // more with a stricter reminder. Whichever pass comes back clean
    // wins; if neither does, the first pass is used but hard-capped
    // so it can never balloon into a wall of text on screen.
    async function requestPhoneGeneration(characterName, mode) {
        ...
        if (firstClean && !looksLikeNarration(firstClean, characterName)) {
            return firstClean;
        }
        ...
        try {
            ...
            if (retryClean && !looksLikeNarration(retryClean, characterName)) {
                return retryClean;
            }
