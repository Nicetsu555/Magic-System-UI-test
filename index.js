...
function createDashboard(optionsMenu) {
    ...
    // generateQuietPrompt() by default drags in the ENTIRE main
    // story log as context — fine for a short chat, but in a long
    // roleplay that's hundreds of messages re-sent (and re-waited-on)
    // just to answer one text message. The phone only needs to know
    // roughly "what's going on right now", not the whole history, so
    // the main chat log is temporarily cut down to its last N
    // messages for the duration of the quiet generation call (see
    // runQuietPhonePrompt below), then put back exactly as it was
    // immediately after.
    const PHONE_STORY_CONTEXT_LIMIT = 12;
    ...
    async function runQuietPhonePrompt(context, prompt, characterName) {
        ...
        // Shrink the LIVE chat array (same object SillyTavern's own
        // generation code reads) down to the last
        // PHONE_STORY_CONTEXT_LIMIT messages for just this one call,
        // so the phone reply is still aware of "what just happened"
        // without dragging the entire story log through the prompt
        // every single time someone sends a text. This mutates the
        // array in place — it does NOT touch chat.json on disk, and
        // is restored (unshifted back) the moment the call settles,
        // success or failure, via the finally block below.
        //
        // NOTE: this narrows the window, it doesn't remove it —
        // if the user triggers a NEW main-story generation, swipe,
        // or edit in the split second the phone call is in flight,
        // it would briefly see the shortened array. In practice the
        // phone call resolves in a few seconds and this is the same
        // trade every "quiet"/background generation makes; it's not
        // a concern for normal one-thing-at-a-time use.
        const chatLog = context && Array.isArray(context.chat)
            ? context.chat
            : null;

        const overflow = (chatLog && chatLog.length > PHONE_STORY_CONTEXT_LIMIT)
            ? chatLog.length - PHONE_STORY_CONTEXT_LIMIT
            : 0;

        const removed = overflow > 0
            ? chatLog.splice(0, overflow)
            : null;
        ...
        try {
            ...
            // Put everything removed back at the front, in its
            // original order, restoring the array to exactly how it
            // was before this call started.
            if (removed && removed.length) {
                chatLog.unshift(...removed);
            }
